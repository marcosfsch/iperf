...
static int udp_sendmsg_gso(int fd, const char *buf, size_t count, uint16_t gso_size, int sock_opt)
{
...
int
Nwrite_gso(int fd, const char *buf, size_t count, int prot, uint16_t gso_size, int sock_opt)
{
	register ssize_t r;

	r = udp_sendmsg_gso(fd, buf, count, gso_size, sock_opt);
